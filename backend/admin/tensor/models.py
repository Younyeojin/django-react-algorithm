...
class TensorFunction(object):
    ...
    def train_tf_model_by_random_data(self):
        ...
        history = model.fit(x_train, y_train, epochs=30, validation_split=0.3)  # validation_split : test(쪼개서 테스트까지 알아서 해라)
    ...
    def create_tf_empty_model(self):  # p.453
        '''
    ...
    def tf_add(self):
        ...
        y = [1, 2, 3, 4, 5]
        # y = [2, 3]
        z = tf.add(x, y)  # z = tf.math.add(x, y)
        # z = tf.subtract(x, y)
        # z = tf.multiply(x, y)
        # z = tf.divide(x, y)
        return z
